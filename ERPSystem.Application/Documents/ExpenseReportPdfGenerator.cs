...
/// <summary>Shared Expense Report PDF renderer for API and WPF.</summary>
public sealed class ExpenseReportPdfGenerator
{
    private static readonly CultureInfo WesternNumbers = CultureInfo.InvariantCulture;
    private readonly string _logoPath;

    public ExpenseReportPdfGenerator(string fontPath, string logoPath)
    {
        ConfigureQuestPdf(fontPath);
        _logoPath = logoPath;
    }

    public static ExpenseReportPdfGenerator FromContentRoot(string contentRoot)
    {
        var (fontPath, logoPath) = ResolveAssets(contentRoot);
        return new ExpenseReportPdfGenerator(fontPath, logoPath);
    }
