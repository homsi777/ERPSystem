...
/// <summary>Shared Payment Voucher (سند صرف) PDF renderer for API and WPF.</summary>
public sealed class PaymentVoucherPdfGenerator
{
    private static readonly CultureInfo WesternNumbers = CultureInfo.InvariantCulture;
    private readonly string _logoPath;

    public PaymentVoucherPdfGenerator(string fontPath, string logoPath)
    {
        ConfigureQuestPdf(fontPath);
        _logoPath = logoPath;
    }

    public static PaymentVoucherPdfGenerator FromContentRoot(string contentRoot)
    {
        var (fontPath, logoPath) = ResolveAssets(contentRoot);
        return new PaymentVoucherPdfGenerator(fontPath, logoPath);
    }
