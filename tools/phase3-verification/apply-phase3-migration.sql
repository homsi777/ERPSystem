-- RECOVERY ONLY (test DB when EF migrate cannot run).
-- Authoritative path: 20260722120000_AddPhase3FinanceModule + 20260722121000_FixPhase3FinanceSchema
-- via: dotnet ef database update
-- Phase 3 migration SQL (partial mirror — use EF migrations when possible)
CREATE TABLE IF NOT EXISTS finance.payment_methods (
    ...
    CONSTRAINT "PK_payment_methods" PRIMARY KEY ("CompanyId", "Id")
);
...
SELECT '20260722120000_AddPhase3FinanceModule', '9.0.6'
WHERE NOT EXISTS (
    SELECT 1 FROM "__EFMigrationsHistory" WHERE "MigrationId" = '20260722120000_AddPhase3FinanceModule'
);
