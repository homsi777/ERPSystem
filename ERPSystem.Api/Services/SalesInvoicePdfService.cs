...
/// <summary>
/// The single server-side Sales Invoice renderer used by every HTTP client.
/// It consumes existing calculated DTO values and never recalculates business totals.
/// </summary>
public sealed class SalesInvoicePdfService
{
    ...
    {
        var fontPath = Path.Combine(environment.ContentRootPath, "Assets", "Fonts", "NotoSansArabic.ttf");
        _logoPath = ResolveLogoPath(environment.ContentRootPath);
        ConfigureQuestPdf(fontPath);
    }

    private static string ResolveLogoPath(string contentRootPath)
    {
    ...
    private static void ComposeLines(IContainer container, IReadOnlyList<SalesInvoiceLineDto> lines)
    {
        container.Table(table =>
        {
            ...
            {
                columns.RelativeColumn(2.6f); // item - rightmost in RTL flow
                columns.RelativeColumn(1.3f); // color
                columns.RelativeColumn(1.05f); // rolls
                columns.RelativeColumn(1.2f); // length
                columns.RelativeColumn(1.35f); // unit price
                columns.RelativeColumn(1.2f); // discount
                columns.RelativeColumn(1.2f); // tax
                columns.RelativeColumn(1.45f); // total
            });
