...
- النتيجة النهائية بعد إضافة اختبارات الإدخال اليدوي والعداد: **9 passed, 0 failed**.
...
- الاختبارات المحلية المركزة: **9 passed, 0 failed**.
...
- الاختبارات المركزة: **9 passed, 0 failed**.
...
## 11. إصلاح سرعة الإدخال وانهيار الحفظ والترحيل

### الخانات الفارغة

- القماش واللون والأمتار وعدد الرولات والتكلفة تظهر فارغة تماماً عند فتح الشاشة.
- بعد إضافة السطر تُفرغ الخانات الخمس بالكامل ويعود التركيز إلى خانة القماش لتسريع إدخال السطر التالي.
- لا توجد قيم افتراضية `0/1/0` داخل خانات السطر.
- عند نسيان الأمتار أو عدد الرولات تظهر رسالة تحقق واضحة بدلاً من تجاهل الضغط على «إضافة» بصمت.

### سبب خطأ الحفظ 23505

الخطأ المرسل كان:

`duplicate key value violates unique constraint "IX_opening_balance_documents_CompanyId_Number"`.

كان عداد `OpeningBalance` أقل من أعلى رقم محفوظ فعلياً، فحاول إنشاء رقم مستند موجود. أُصلح مولد الأرقام ليقرأ أعلى رقم موجود للشركة والفرع، ويرفع العداد إليه قبل حجز الرقم التالي. كما أضيف اختبار يعيد العداد للخلف عمداً ويزرع الرقم المتوقع مسبقاً؛ نجح النظام في تجاوزه وتوليد الرقم التالي.

### منع انهيار الواجهة

- يُمنع تكرار الضغط على «حفظ المستند» أثناء عملية الحفظ نفسها.
- يُمنع تكرار الضغط على «ترحيل للمخزون» أثناء عملية الترحيل.
- أخطاء قاعدة البيانات في إنشاء المستند تعاد كنتيجة فشل مفهومة.
- أضيف حاجز أخطاء في الشاشة يعرض رسالة عربية ولا يسمح للاستثناء غير المعالج بإسقاط التطبيق.

### عيب الترحيل الذي كشفه الاختبار

كشف اختبار الدورة الكاملة أن مستند المخزون الوسيط وأسطره يكونان مضافين حديثاً إلى EF Change Tracker، بينما `PostOpeningStockAsync` كان يبحث عنهما في قاعدة البيانات فقط قبل تنفيذ `SaveChanges`؛ لذلك كان يرمي `Opening stock document not found`. أُصلح المسار ليقرأ الكيانات الجديدة المتتبعة أولاً ثم يرجع إلى قاعدة البيانات للمستندات القديمة. وعُدّل تحديث `WarehouseStocks` بالطريقة نفسها لمنع تكرار سجل المخزون عند وجود أكثر من سطر مطابق في الدفعة.

### التحقق النهائي

- بناء Release: **PASS — 0 errors, 0 warnings**.
- الاختبارات المركزة: **9 passed, 0 failed**.
- اختبار PostgreSQL حقيقي: إدخال يدوي بلا IDs → حفظ المستند → Submit → Approve → Post/Lock → إنشاء 5 رولات legacy → rollback وتنظيف كامل: **PASS**.
- اختبار عداد متأخر مع رقم مستند مزروع مسبقاً: **PASS**.
- لم يتصل Codex بالسحابة ولم يشغّل WPF.

**الحالة الأحدث: الخانات فارغة وسريعة، سبب 23505 مُعالج، انهيار الواجهة ممنوع، ودورة الحفظ والترحيل الكاملة نجحت محلياً. التعديلات تنتظر الرفع وإعادة النشر.**
