; Inno Setup — مثبّت احترافي لسطح المكتب (اتصال مباشر SSL بدون SSH)
...
begin
  DbPasswordPage := CreateInputQueryPage(wpSelectDir,
    ...
    'اتصال مباشر وآمن بالسحابة (بدون SSH)',
    'أدخل كلمة مرور مستخدم قاعدة البيانات erp_app.' + #13#10 +
    'يتصل التطبيق مباشرة بـ IP السيرفر (بدون Cloudflare) — لا حاجة لنفق SSH.');
...
begin
  ...
  begin
    ...
    Json :=
      ...
      '    "DefaultConnection": "Host={#DbHost};Port=5432;Database=erp_pro;Username=erp_app;Password=' + Password + ';SSL Mode=Prefer;Trust Server Certificate=true"' + #13#10 +
      '  },' + #13#10 +
      '  "SshTunnel": { "Enabled": false },' + #13#10 +
      '  "Desktop": { "ConnectionMode": "CloudDirect" }' + #13#10 +
      '}';
